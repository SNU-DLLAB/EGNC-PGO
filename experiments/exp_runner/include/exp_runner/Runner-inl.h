...
/***************************************************************************/
template <class POSE_TYPE>
void ExperimentRunner<POSE_TYPE>::run(irl::Log& log, std::string output_dir, size_t mode) {
  method_->init();

  // set mode if the method is risam
  if (method_->getName() == "risam") {
    method_->set_mode(mode);
  }
  ...
  for (auto& entry : log.entries) {
    ...
    std::cout << "Total Execution Time (s): " << total_execution_time_s_ << std::endl;
  ...
  *iteration_times_file << total_execution_time_s_ << std::endl;  // add total time to end of file
