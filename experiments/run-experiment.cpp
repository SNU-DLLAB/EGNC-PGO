...
po::variables_map handle_args(int argc, const char *argv[]) {
  ...
  options.add_options()
      ...
      ("mode,d",        po::value<size_t>()->required(),        "(Required) 0: original risam, 1: B-spline without adaptive, 2: our adaptive method");
  // clang-format on
  ...
  if (var_map.count("help") || argc == 1) {
    std::cout << "run-experiment: Main entry point to run robust SLAM methods on IRL datasets. Please provide required "
                 "arguments: "
              << std::endl;
    std::cout << options << "\n";
    exit(1);
...
int main(int argc, const char *argv[]) {
  ...
  std::cout << "----------------------------------" << std::endl;
  std::cout << "Mode :" << args["mode"].as<size_t>() << std::endl;
  std::cout << "----------------------------------" << std::endl;
  ...
  if (log.header.linearity == "nonlinear") {
    if (log.header.global_dim == 2) {
      ...
      runner.run(log, args["output_dir"].as<std::string>(), args["mode"].as<size_t>());
